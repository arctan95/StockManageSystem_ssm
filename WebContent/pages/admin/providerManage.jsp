...
<script type="text/javascript">
	...
	$.messager.confirm("系统提示","您确认要删除这<font color=red>"+selectedRows.length+"</font>条数据吗?",function(r){
		if(r){
			$.post("${pageContext.request.contextPath}/provider/delete",{delIds:ids},function(result){
				if(result.success){
					$.messager.alert("系统提示","您已成功删除<font color=red>"+result.delNums+"</font>条数据！");
					$("#dg").datagrid("reload");
				}else{
					$.messager.alert("系统提示","<font color=red>"+selectedRows[result.errorIndex].proName+"</font>"+result.errorMsg);
				}
			},"json");
		}
	});
	...
	function openProviderAddDialog(){
		...
		url="${pageContext.request.contextPath}/provider/save";
	}
	...
	function openProviderModifyDialog(){
		...
		url="${pageContext.request.contextPath}/provider/save?id="+row.id;
	}
	...
	function exportData(){
		window.open('${pageContext.request.contextPath}/provider/export')
	}
</script>
</head>
<body style="margin:5px;">
	<table style="height:423px;width:1160px" id="dg" title="供应商管理" class="easyui-datagrid" fitColumns="true"
	pagination="true" rownumbers="true" url="${pageContext.request.contextPath}/provider/list" toolbar="#tb">
		<thead>
			<tr>
				<th field="cb" checkbox="true"></th>
				<th field="id" width="15">编号</th>
				<th field="proId" width="15">供应商编号</th>
				<th field="proName" width="25">供应商名称</th>
	...
	<div id="tb">
		...
		<div>&nbsp;供应商名：&nbsp;<input type="text" id="s_proName" onkeydown="if(event.keyCode == 13)searchProvider()"/><a href="javascript:searchProvider()" class="easyui-linkbutton" iconCls="icon-search" plain="true">搜索</a>
	...
	closed="true" buttons="#dlg-buttons">
		<form id="fm" method="post">
			<table cellspacing="5px;">
				<tr>
					<td>供应商编号：</td>
					<td><input type="text" name="proId" id="proId" class="easyui-validatebox" required="true"/></td>
					
					<td>供应商名：</td>
					<td><input type="text" name="proName" id="proName" class="easyui-validatebox" required="true"/></td>
				</tr>
				<tr>
					<td>联系人：</td>
					<td><input type="text" name="linkman" id="linkman" class="easyui-validatebox" required="true"/></td>
					
					<td>联系电话：</td>
					<td><input  name="proPhone" id="proPhone" class="easyui-validatebox" required="true" /></td>
				</tr>
				<tr>
					<td valign="top">供应商备注：</td>
					<td colspan="3"><textarea rows="7" cols="45" name="proDesc" id="proDesc"></textarea></td>
				</tr>
			</table>
		</form>
	</div>
