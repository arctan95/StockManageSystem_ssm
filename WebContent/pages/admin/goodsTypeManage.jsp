...
<script type="text/javascript">
	...
	function deleteGoodsType(){
		...
		$.messager.confirm("系统提示","您确认要删除这<font color=red>"+selectedRows.length+"</font>条数据吗?",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/goodsType/delete",{delIds:ids},function(result){
					if(result.success){
						$.messager.alert("系统提示","您已成功删除<font color=red>"+result.delNums+"</font>条数据！");
						$("#dg").datagrid("reload");
					}else{
						$.messager.alert("系统提示","<font color=red>"+selectedRows[result.errorIndex].typeName+"</font>"+result.errorMsg);
					}
				},"json");
			}
		});
	...
	function openGoodsTypeAddDialog(){
		...
		url="${pageContext.request.contextPath}/goodsType/save";
	}
	...
	function openGoodsTypeModifyDialog(){
		...
		url="${pageContext.request.contextPath}/goodsType/save?id="+row.id;
	}
	...
	function exportData(){
		window.open('${pageContext.request.contextPath}/goodsType/export')
	}
</script>
</head>
<body style="margin: 5px;">
	<table style="height:423px; width:1160px;" id="dg" title="商品类别管理" class="easyui-datagrid" fitColumns="true"
	 pagination="true" rownumbers="true" url="${pageContext.request.contextPath}/goodsType/list" toolbar="#tb">
		<thead>
			<tr>
	...
	<div id="tb">
		...
		<div>&nbsp;商品类别名称：&nbsp;<input type="text" name="s_typeName" id="s_typeName" onkeydown="if(event.keyCode == 13)searchGoodsType()"/><a href="javascript:searchGoodsType()" class="easyui-linkbutton" iconCls="icon-search" plain="true">搜索</a>
		<a href="javascript:clearValue()" class="easyui-linkbutton" iconCls="icon-no" plain="true">清空</a></div>
	</div>
	...
	closed="true" buttons="#dlg-buttons">
		<form id="fm" method="post">
			<table cellspacing="5px;">
				<tr>
					<td>商品类别名称：</td>
					<td><input type="text" name="typeName" id="typeName" class="easyui-validatebox" required="true"/></td>
				</tr>
				<tr>
					<td valign="top">商品类别备注：</td>
					<td colspan="3"><textarea rows="7" cols="45" name="typeDesc" id="typeDesc"></textarea></td>
				</tr>
			</table>
		</form>
	</div>
